...
struct Move {
  ...
  constexpr Move(Cords start, Cords end, Move::Type type = Type::NORMAL)
      : data_(static_cast<std::uint16_t>(start.getIndex()) |
              (static_cast<std::uint16_t>(end.getIndex()) << 6) |
              (static_cast<std::uint16_t>(type) << 12)) {}

  constexpr Move(std::uint8_t startI, std::uint8_t endI,
                 Move::Type type = Type::NORMAL)
      : data_(static_cast<std::uint16_t>(startI) |
              (static_cast<std::uint16_t>(endI) << 6) |
              (static_cast<std::uint16_t>(type) << 12)) {}

  [[nodiscard]] constexpr Cords getStartSquare() const {
    return Cords(static_cast<std::uint8_t>(data_ & 0x3F)); // Mask bits 0-5
  }
  [[nodiscard]] constexpr Cords getEndSquare() const {
    return Cords(static_cast<std::uint8_t>(
        (data_ >> 6) & 0x3F)); // Shift right 6, mask bits 6-11
  }
  ...
  [[nodiscard]] constexpr std::string getMoveString() const {
